...
/**
 * SEO Title: HEIC to JPG Converter - Convert Apple Photos Online
 * Meta Description: Easily convert Apple HEIC photos to JPG format. Fast, free, and secure client-side conversion. Batch processing support.
 */

import React, { useState, useEffect } from 'react';
import { Download, Loader2, Zap, AlertCircle } from 'lucide-react';
...
export default function HeicToJpg() {
  ...
  useEffect(() => {
    ...
    // Dynamically load heic2any from CDN for zero-dependency build
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      console.error(err);
      alert("Conversion failed. Ensure files are valid HEIC photos.");
    } finally {
    ...
    <div className="w-full p-4 md:p-8 space-y-6">
      ...
      {files.length > 0 && results.length === 0 && (
        ...
        >
          ...
          Convert {files.length} HEIC Photos to JPG
        </button>
      )}
      ...
      {results.length > 0 && (
        <div className="card border rounded-xl p-6 bg-card shadow-sm space-y-6 animate-in slide-in-from-bottom-4 duration-500">
          <div className="flex items-center justify-between">
            <h3 className="text-xs font-black uppercase tracking-widest text-primary">Converted JPGs</h3>
