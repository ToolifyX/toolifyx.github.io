...
export class PostHogProvider implements IAnalyticsProvider {
  name = 'PostHog';
  private key = process.env.NEXT_PUBLIC_POSTHOG_KEY;
  private host = process.env.NEXT_PUBLIC_POSTHOG_HOST || 'https://app.posthog.com';

  async init(): Promise<void> {
    if (!this.key || typeof window === 'undefined') return;
